...
> 状态：v1.1.0 发布候选已完成；v1.2.0 扩展项继续实施  
> 计划版本：1.0  
> 目标产品版本：1.1.0  
> 最近核验：2026-08-06  
...
- [x] 冻结并以 LLM 标注来源分层的 300 份以上公告/候选样本及 100 份以上活动记录；候选召回、固定案例、
  实体精确率和集团去重门槛已验证。原文列名召回作为已知限制单独报告；2026-08-10 用户决定不再进行真实
  人工抽检，并接受其不作为 v1.1.0 发布阻塞（所有对外声明必须注明“LLM 标注口径”）。
...
2026-08-08 用户明确授权评估集由 LLM 标注（代理使用 DeepSeek API 生成标签并冻结）。
2026-08-10 用户进一步决定不再进行真实人工抽检，v1.1.0 以全量 LLM 冻结集、固定回归、
生产数据库只读副本比较、离线/live 测试和构建结果作为发布证据。任何对外声明必须如实
标注“LLM 标注口径”，不得表述为“人工核验”；LLM 指标未达原目标时必须公开具体数值和
已知风险，不能伪称达标。
...
1. v1.1.0 发布范围内的里程碑复选框均有自动化测试、LLM 冻结评估或生产副本证据支持；明确移交 v1.2.0 的扩展项不阻塞本版。
...
> 状态：v1.1.0 范围已收尾；未完成的覆盖中心、OCR 与自动补账移交 v1.2.0
...
### v2 里程碑 2：扩源、正文与覆盖同步完善（剩余项移交 v1.2.0，不阻塞 v1.1.0）
...
- [x] 先定位“参与单位/参会机构/投资者名单”章节，再解析表格、跨行字段、编号列表、中英文混排和机构—人员组合；
  取消面向整篇正文的宽泛兜底提取。
- [x] 支持英文机构后缀及常见结构，如 `Capital`、`Asset Management`、`Securities`、`Fund`、`Partners`、
  `Bank`、`Insurance`、`Trust`、`Advisors`。
- [x] 中文短名只通过种子别名、原文全称关联或高置信规则归一；模糊相似继续仅进入待复核，禁止自动合并。
- [x] 组织分类先于计分：上市公司自身、产业公司、律所、媒体、个人和描述性短语不进入研究机构广度。
- [x] 历史 550 天活动按新解析版本可恢复重算；每份活动原子替换旧参与者，整个指标批次完成后才发布新的 20/60/120 日榜。
...
- 机构冻结集不少于 200 份活动记录，并覆盖三所、中文/英文、表格/跨行/仅总数和零列名场景：
  - 研究机构实体精确率目标 `≥92%`；列名召回继续报告原始数值，2026-08-10 用户接受其作为已知限制、不作为 v1.1.0 发布阻塞。
  - 全部组织提及精确率与召回率均 `≥90%`。
  - 集团去重精确率 `≥95%`。
  - 固定集中上市公司自身、媒体、个人和描述短语进入主指标的数量为零。
- 全量使用已授权的 LLM 标注口径，不再要求真实人工抽检。发布说明必须逐项报告 LLM 指标、标注模型、样本规模与未达目标，
  不得用模拟人工纠正结果替代真实证据或把 LLM 结果表述为人工核验。
...
发布决策审计（2026-08-10·用户明确决定）：用户决定不再执行真实人工抽检，并要求清除 v1.1.0
Release 前阻塞。发布评估固定为全量 LLM 标注口径：600 事件的 Precision@10=1.000、Top20
无关=0、重复=0、独立必达召回=0.882；386 发现候选召回=1.000、固定案例零遗漏；200 活动的
研究实体精确率=0.967、全部组织实体精确率=0.916、全部组织列名召回=0.910、集团去重=1.000。
研究机构列名召回=0.857，低于原 0.92 目标，用户接受其作为 v1.1.0 已知风险并取消该项发布阻塞；
对外不得宣称该指标达标或经过人工核验。M4 五项实现行为已有 fixture、全量离线、550 天原子重算和
生产副本证据，复选框据此勾选。v2 M2 尚未完成的 OCR、完整覆盖中心和自动补账明确移交 v1.2.0，
不在 v1.1.0 声称范围内；扫描件继续失败关闭并保留候选/原文链接。
...
# 第四部分：机构升温科学性修正与 warming v2

> 状态：实施中  
> 计划版本：1.0（2026-08-10 用户确认实施）  
> 数据库目标：schema 121 → 122  
> 发布原则：科学性优先于榜单数量；覆盖、时间语义或机构资格不足时返回空值，不生成看似完整的排名

本部分修正第三部分既有机构升温口径。目标仍是描述公开披露的机构研究行为变化，
不表示看多、买入、持仓、资金流向，不预测股价或收益。

## 机构升温 v2 里程碑

### 里程碑 0：发生日、来源 cohort 与版本化存储底座

- [x] 新增 `ActivityOccurrence` 与 `ResearchParticipantOccurrence`，分别保存可靠活动日/区间、日期精度、
  指标资格和“机构—日期”关系；分析师身份由“机构 ID + 姓名”共同表达。
- [x] 新增逐来源、逐市场、逐窗口的 `SourceWindowCoverage`，保存共同覆盖判断所需的成功、对账、回填边界、
  cohort 资格、错误和排除原因。
- [x] SQLite 以加法式 121→122 迁移新增 occurrence/coverage 表，并为机构指标快照增加
  `metric_version`/`source_cohort_id`；创建一次性不覆盖 `pre-122` 备份，失败整体回滚。
- [x] 旧活动日期、旧参与者、旧快照和 `z20` 保持可读；旧指标行默认标记 `z20_legacy`，新旧读取接口并存。
- [x] 新表进入级联清理、`clear_all`、存储统计与诊断；覆盖新库、121 升级、重复初始化、失败回滚和 CRUD 往返测试。

验收：schema 121 原位升级到 122；迁移失败仍保留 121 可用；legacy 指标读取不变；新数据类型往返稳定；
相关测试和全量离线通过。

审计记录（2026-08-10·里程碑 0）：新增四个版本化公共契约及三张表；occurrence 父子行按单活动原子替换，
错误的跨活动/悬空机构日期映射在写入事务前拒绝；被排除组织仍保留在明细表，指标消费者可只读取
`research_eligible=1`。`institution_metric_snapshots` 旧行迁移后自动带 `metric_version=z20_legacy`、
空 `source_cohort_id`，旧 tuple/payload 接口未改变，新增版本感知读取接口供 warming v2 使用。
相关模型/存储/机构指标测试 `107 passed`；全量离线 `603 passed, 13 skipped`。未运行 live、Windows onedir、
550 天重算或生产副本比较（本里程碑不接入解析器、不切换指标、不访问网络）。默认榜仍使用 legacy 口径。

### 里程碑 1：活动时间语义与机构资格

- [x] 日期只从“时间/日期/活动安排”字段或名单表格解析；正文其他日期不进入活动时间；未来误日期为零。
- [x] 无法确定具体日期时保留区间或披露日，但不参与重复跟进、活跃日和集中度计算。
- [x] 主指标只消费 `research_eligible=True`；企业、媒体、律所、咨询机构保留明细但不计广度。
- [x] 银行仅在明确为研究部门时计入；信托建立明确角色口径；英文 `Limited/Ltd` 不再自动等于境外研究机构。
- [x] 冻结日期干扰、跨日名单和机构分类案例；研究机构精确率与召回率均达到 92%（LLM 标注口径须如实注明）。

审计记录（2026-08-10·机构升温 v2 里程碑 1）：①`research_activities.py` 新增版本化 occurrence 解析：
只扫描“活动时间/日期/活动安排/日程安排”等结构化字段和名单日期表；Q&A/经营正文中的报告期、计划日等日期
不进入活动时间，晚于披露日的未来日期失败关闭。明确单日写 `explicit_day + metric_eligible=1`；区间只保留
`period_start/period_end` 一行（不逐日枚举），无可靠日期保留披露日，两者均 `metric_eligible=0`。跨日名单仅在
机构所在表格/名单行带明确日期时建立机构—日期关系；多日但无法逐机构映射时保留参与者明细，不生成悬空映射。
②参与者 occurrence 以“机构 ID + 分析师姓名 + 活动日”表达身份；同名分析师在不同机构不合并，同一机构的
多名明确分析师均保留。解析服务在 v2 路径原子替换 occurrence 父子行，v1 回退路径与旧活动日期/参与者继续可读。
③机构资格先于指标：券商、基金、保险、资管、私募及明确境外投资机构计入；企业、媒体、律所、咨询/专业服务
组织保留明细并写明排除原因；银行只有正文/名单明确“研究部/研究所/投研”等角色时计入，信托只有明确证券投资/
权益投资/投研角色时计入；英文 `Limited/Ltd` 仅视为法律后缀，`DM Capital Limited`/`Asset Management Ltd`
等有研究机构词形者仍可计入。指标加载器检测到 v2 occurrence 后只消费可靠单日与 `research_eligible=1`，
未重算的旧活动继续走 legacy 兼容读路径。④冻结回归：日期干扰、未来日期、区间/披露日降级、跨日名单、
分析师身份及机构分类均在 `tests/test_warming_v2_m1.py`；30 条机构资格 fixture 按**LLM 标注口径**冻结，
精确率 1.000、召回率 1.000（门槛均为 0.92；小型规则契约集，不代表生产全量人工评估）。相关模型/存储/
机构解析/指标回归 `79 passed`；全量离线 `610 passed, 13 skipped`。未运行 live、Windows onedir、550 天生产
重算或生产副本对比（本里程碑不改来源契约、不切换默认指标）；默认仍为 `z20_legacy`，后续 cohort、
`warming_v2` 公式、持续关注修正和产品切换分别留在里程碑 2–5。

### 里程碑 2：市场适用来源、共同覆盖与 cohort

- [x] 按沪、深、北分别定义适用的 `research_activity` 来源；公告源状态不污染机构榜。
- [x] 共同覆盖使用最晚 `covered_start` 与最早 `covered_end`，结束日不晚于当前交易日；来源须成功、无未关闭错误、
  已到回填边界。
- [x] 历史不足的新来源同时从当前桶和基线排除，以补充广度独立展示，达到稳定条件后才进入同一 cohort。
- [x] 覆盖不足时使用“20 日机构关注（冷启动）”，不展示正式升温排名。

审计记录（2026-08-10·机构升温 v2 里程碑 2）：新增逐市场 cohort 构建器，固定适用关系为沪市
`sse_publish`、深市 `cninfo_research + irm_ircs`、北市 `bse_performance`；三类公告源不进入机构覆盖分母、
成功计数或错误状态。`warming_20` 请求窗口固定为当前 20 日加此前 12 个非重叠 20 日桶（260 个交易日）；
逐来源只有成功、无未关闭错误、覆盖到请求起点且最新扫描覆盖到当前交易日时才具备 cohort 资格，来源结束日
强制截断到当前缓存交易日。共同覆盖严格取 cohort 来源的最晚起点和最早终点。新来源历史不足时记录为
`supplemental_source_keys` 并独立显示，不进入可比 cohort；一旦进入，成员资格保持粘性，后续来源失败会把该市场
标记为不可正式排名，不能静默缩小来源集合。机构视图按股票所属市场门控：不可比行仅保留原始关注指标，升温值
置空；没有正式行时标题固定为“20 日机构关注（冷启动）”。旧 `z20_legacy` 计算/存储保持兼容，未提前切换默认
指标。新增 `tests/test_warming_v2_m2.py` 5 项冻结契约；相关模型/存储/机构服务/视图回归 `72 passed`，全量离线
`615 passed, 13 skipped`。未运行 live、Windows onedir、550 天重算或生产副本比较（本里程碑不访问网络，
也不完成 `warming_v2` 公式和发布切换）。

### 里程碑 3：描述性标准化升温值

- [x] 保留 `z20_legacy` 一个版本周期；新增 `warming_v2`，完整值使用当前 20 日桶和此前 12 个非重叠 20 日桶。
- [x] 5–11 个历史桶只生成暂定值；少于 5 个仅展示原始指标。
- [x] 使用样本方差（ddof=1），`predictive_variance=max(baseline_mean, baseline_variance, 1)×(1+1/n)`，
  `warming_score=(current-baseline_mean)/sqrt(predictive_variance)`；名称固定为“标准化升温值（描述性）”。
- [x] 排序固定为：完整覆盖 → warming_score → 100 日未见集团数 → 活跃日期数 → 当前集团数 → 最近活动降序 → 股票代码。
- [x] 同时展示绝对增量、当前集团数、活跃日期数和单日集中度；单一活动日标记“单日集中”。
- [x] 行业分位数只有完整上市公司行业宇宙（含零活动股票）且样本不少于 20 时计算，否则为空且不参与排序。

审计记录（2026-08-10·机构升温 v2 里程碑 3）：新增版本化 `WarmingV2Row` 与透明公式组件；
`warming_20` 使用尾部完整交易日组成的当前 20 日桶及最多 12 个此前非重叠 20 日桶，方差严格为样本方差
（`ddof=1`），预测方差与标准化升温值按本节固定公式计算并保存均值、方差、预测方差和绝对增量。12 个历史桶
且市场 cohort 完整时为 `full`；5–11 桶为 `provisional`；少于 5 桶或既有 cohort 来源当前不可用时为
`raw_only`，标准化值为空。排序实现完整/暂定/原始层级后依次使用升温值、100 日未见集团数、活跃日期数、
当前集团数、最近活动降序和股票代码兜底。行同时保存当前集团数、100 日未见集团数、活跃日期数、单日集中度
及“单日集中”标记。行业分位数接口必须由调用方显式提供完整上市公司行业宇宙，分母包含零活动股票且行业样本
不少于 20；普通观察股票缓存不能证明宇宙完整，生产并行路径因此诚实返回空分位数。服务同一原子发布批次并行写入
`window_kind=warming_20`、`metric_version=warming_v2`、真实 `source_cohort_id`，继续保留 `z20_legacy`。
新增 `tests/test_warming_v2_m3.py` 5 项冻结公式、降级、排序、零活动行业分母及并行快照测试；相关回归
`82 passed`，全量离线 `620 passed, 13 skipped`。首次全量命令仅因 120 秒执行上限在约 90% 处终止，
以 240 秒上限原样重跑通过。未运行 live、Windows onedir、550 天重算或生产副本比较；默认 UI 尚未在本里程碑
切换到 `warming_v2`，由里程碑 5 灰度开关控制。

### 里程碑 4：持续关注规则指数

- [x] `unique_groups=0`、机构—日期映射缺失或日期不可靠时总分返回 `None`，不得因 concentration=0 获得分数。
- [x] 问答正文缺失与低深度问题分开表达；缺失数据不按零分混入完整指数。
- [x] 四组件继续单独展示；总分名称改为“持续关注规则指数”，独立标签校准前不赋予统计含义。

审计记录（2026-08-10·机构升温 v2 里程碑 4）：新增 `PersistenceRuleComponents` 与
`persistence_rule_index`，四个透明组件继续独立保存；`unique_groups=0`、任一纳入活动缺少可靠机构—日期映射、
来源共同覆盖不足，或研究问答数据不完整时总分严格返回 `None`，不会再从 `concentration=0` 获得虚假加分。
问答状态固定区分 `available`、`missing_body` 与 `partial_missing`：有正文且问题均为低深度时深度组件按既有低深度
权重计算，问答缺失则深度组件和总指数均为空。服务在同一发布批次并行保存 `persistence_60_v2`/
`persistence_120_v2`，快照标记 `metric_version=persistence_rules_v2`，并保留活跃周比例、重复跟进比例、研究深度、
单日集中度、日期映射质量、问答状态及明确暂定原因；旧 `persistence_60/120` 继续作为一个版本周期的回退口径。
新增 `tests/test_warming_v2_m4.py` 5 项缺失/低深度、零机构、日期映射、部分缺失、并行发布与序列化冻结测试；
相关回归 `125 passed`，全量离线 `625 passed, 13 skipped`。未运行 live、Windows onedir、550 天生产重算或
生产副本比较；专业窗口中的正式名称与全出口切换留在里程碑 5 的统一灰度开关，避免半切换。

### 里程碑 5：历史重算、产品切换与最终质量门

- [x] 550 天历史以 v2 解析器进入 staging，全部完成后原子发布，禁止混合新旧解析版本。
- [x] UI、CSV、复制和详情统一展示指标版本、来源 cohort、日期质量、排除组织数和暂定原因。
- [x] 冻结高基数零方差、稀疏零计数、异常历史桶、行业零活动股票、无机构活动和最近日期排序案例。
- [x] 在生产数据库只读副本并行比较 legacy/v2，报告 Top20 变化、非研究组织污染率、日期纠正数和单日集中占比；
  不做股价或收益回测。
- [x] 相关测试、全量离线、显式 live 契约和 Windows onedir 构建全部通过后才切换默认指标；legacy 保留一个版本周期。

审计记录（2026-08-10·机构升温 v2 里程碑 5）：默认产品指标已切换为 `warming_v2` /
`persistence_rules_v2`，设置项仍可显式回退 `z20_legacy` 一个版本周期；专业窗口、CSV、复制和详情统一使用
版本化快照，并展示来源 cohort、日期质量、排除组织数、覆盖状态与暂定原因。550 天重算先把全部活动解析到内存 staging，
再以单个 SQLite 事务原子替换活动、日期、证据、参与者、机构提及、活动 occurrence、参与 occurrence 和披露总数；
任一解析或写入失败不发布半批数据。生产数据库原文件未改动，在只读副本上完成重算：扫描 3443 份文档、持久化
3439 个活动、写入 30310 条参与记录、错误 0；当前本地公开来源历史仅覆盖约 2026-05 至 2026-08，各市场共同
覆盖不足 120/260 交易日，因此 v2 正确保持冷启动，未生成伪正式行。只读并行报告显示 legacy Top20=20、v2
Top20=0（全部因冷启动退出，重叠 0），非研究组织 occurrence 为 5418/28313（19.1361%），可比 1108 个活动中
日期纠正 286 个；v2 正式行 0，单日集中占比因此为空，且明确未做股价/收益回测。报告保存于
`evaluation/warming_v2_recompute_20260810.json` 与 `evaluation/warming_v2_comparison_20260810.json`。
新增 `tests/test_warming_v2_m5.py` 6 项，冻结灰度回退、同日 day/range occurrence 去重、高基数零方差、稀疏零计数、
异常桶、行业零活动股票、无活动、最近日期排序、全出口一致性、失败不发布和只读报告；v2 关键回归 `20 passed`，
全量离线 `631 passed, 13 skipped`，显式联网 live 契约 `13 passed`，Windows 11 x64 PyInstaller 6.21.0
onedir 构建成功，产物位于 `dist/AshareHotPot`。README 已同步 v2 公式、冷启动和规则指数语义；legacy 未删除。

发布收尾审计（2026-08-10·v1.1.0 Release 候选）：按用户取消真实人工抽检的明确决定，README、项目介绍、
评估说明与 RELEASE_NOTES 已统一为全量 LLM 标注口径，并公开研究机构列名召回 0.857 未达原 0.92 目标的
已知风险；不再包含“人工门槛未完成、不得发布”的矛盾声明。v1.1.0 范围外的 OCR、覆盖中心和自动补账已明确
移交 v1.2.0。验证结果：评估/机构升温关键回归 `98 passed`；全量离线 `632 passed, 13 skipped`；首次沙箱
live 运行因 WinError 10013 被禁止联网（不计代码/站点失败），获准联网后 `12 passed, 1 failed`，唯一失败为
互动易活动流第 2 页瞬时读取超时，定向重跑 `1 passed`，随后完整复跑 `13 passed`。Windows 11 x64、Python
3.12.7、PyInstaller 6.21.0 onedir 构建成功，Inno Setup 6.7.3 安装包构建成功；打包 EXE 在隔离数据目录
offscreen 启动，初始化 schema 122，SQLite `integrity_check=ok`。安装包位于
`dist/installer/AshareHotPot-Setup-1.1.0-x64.exe`（44,433,944 字节，SHA-256
`F9697358710D5964222508D17B764799DCFDFD58092BD07E355A56334CD7089E`），校验清单位于同目录。
安装包另在系统临时目录完成静默安装、隔离数据启动（schema 122、`integrity_check=ok`）和静默卸载冒烟，
